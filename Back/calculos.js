...
function getPrediccionAlumno(
    ...
    let listaTotalAlumnosSecciones = estadisticasAsignaturaPorSeccion.map(
        (e) => {
    ...
    // Recomendacion de la solicitud en base a la probabilidad y prerrequisitos aprobados
    ...
    if (
        ...
        if (
            args.datosAsignatura.prerrequisitos.filter((e) => e.nota >= 5)
                .length >= 1
        ) {
            variables.push({
...
function getProbabilidadAprobacion(
    ...
    let notaCalculo;
    if (
        estadisticasAsignaturasSimilares.listaAsignaturasSimilares.length == 0
    ) {
...
function calcularCorrelacion(datos1, datos2) {
    if (datos1.length !== datos2.length) {
        throw new Error(
            "Ambos conjuntos de datos deben tener la misma longitud"
        );
    }
